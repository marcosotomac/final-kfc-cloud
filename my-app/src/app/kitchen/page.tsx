...
function KitchenContent() {
  ...
  const loadOrders = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await apiService.listOrders(tenantId, "kitchen");
      setOrders(data);
    } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {orders.length === 0 ? (
              ...
              orders.map((order) => (
                <div
                  key={order.orderId}
                  className="border-2 border-orange-300 rounded-lg p-6 bg-orange-50 hover:shadow-lg transition-shadow"
                >
                  <div className="mb-4">
                  ...
                  <div className="mb-4">
                    ...
                    <ul className="space-y-1">
                      {order.items.map((item, idx) => (
                        ...
                        >
                          <span className="inline-block bg-orange-600 text-white rounded-full w-8 h-8 text-center leading-8 mr-2">
                            {item.quantity}
                          </span>
                          {item.name}
                        </li>
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="mt-4 w-full px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:bg-gray-400"
                  >
